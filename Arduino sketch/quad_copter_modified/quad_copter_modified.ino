...
#include "vars.h"   // Include the header file that contains the variables.
...
void setup()
{
    ...
    for (start = 0; start <= 35; start++)
        eeprom_data[start] = EEPROM.read(start); //Read EEPROM for fast access data.
    ...
    //Waiannoyinglyt until the receiver is active and the throtle is set to the lower position.
    while (receiver_input_channel_3 < 990 || receiver_input_channel_3 > 1020 || receiver_input_channel_4 < 1400)
    {
        receiver_input_channel_3 = convert_receiver_channel(3); //Convert the actual receiver signals for throttle to the standard 1000 - 2000us
        receiver_input_channel_4 = convert_receiver_channel(4); //Convert the actual receiver signals for yaw to the standard 1000 - 2000us
        start++;                                                //While waiting increment start whith every loop.
        //We don't want the esc's to be beeping . So let's give them a 1000us puls while waiting for the receiver inputs.
        PORTD |= B11110000;      //Set digital poort 4, 5, 6 and 7 high.
        delayMicroseconds(1000); //Wait 1000us.
        PORTD &= B00001111;      //Set digital poort 4, 5, 6 and 7 low.
        delay(3);                //Wait 3 milliseconds before the next loop.
    ...
    // battery_voltage = (analogRead(0) + 47) * 1.7032;
...
void loop()
{
    ...
    /*---X axis angle---*/
    Total_angle[0] = 0.98 * (Total_angle[0] + gyro_roll_input * elapsedTime) + 0.02 * Acceleration_angle[0];
    /*---Y axis angle---*/
    Total_angle[1] = 0.98 * (Total_angle[1] + gyro_pitch_input * elapsedTime) + 0.02 * Acceleration_angle[1];
    //Serial.print("angleX:   ");Serial.print(Total_angle[0]);Serial.print("       angleY:   ");Serial.println(Total_angle[1]);
    ...
    if (start == 1 && receiver_input_channel_3 < 1050 && receiver_input_channel_4 > 1450)
    {
        ...
        desired_angle_Y = 0;
        pid_yaw_setpoint = 0;
        pid_output_roll_angle = 0;
        pid_output_pitch_angle = 0;
        pid_i_mem_roll_angle = 0;
        pid_last_roll_angle_d_error = 0;
        pid_i_mem_pitch_angle = 0;
        pid_last_pitch_angle_d_error = 0;
    }
    ...
    desired_angle_X = 0;
    //We need a little dead band of 16us for better results.
    if (receiver_input_channel_1 > 1540)
        desired_angle_X = (receiver_input_channel_1 - 1540) / 15.0;
    else if (receiver_input_channel_1 < 1460)
        desired_angle_X = (receiver_input_channel_1 - 1460) / 15.0;
    ...
    desired_angle_Y = 0;
    //We need a little dead band of 16us for better results.
    if (receiver_input_channel_2 > 1540)
        desired_angle_Y = (receiver_input_channel_2 - 1540) / 15.0;
    else if (receiver_input_channel_2 < 1460)
        desired_angle_Y = (receiver_input_channel_2 - 1460) / 15.0;
    ...
    { //Do not yaw when turning off the motors.
        if (receiver_input_channel_4 > 1540)
            pid_yaw_setpoint = (receiver_input_channel_4 - 1540) / 3.0;
        else if (receiver_input_channel_4 < 1460)
            pid_yaw_setpoint = (receiver_input_channel_4 - 1460) / 3.0;
    }
    ...
    // battery_voltage = battery_voltage * 0.92 + (analogRead(0) + 47) * 0.136256;

    //Serial.print(gyro_axis_cal[1]);Serial.print("   ");Serial.print(gyro_axis_cal[2]);Serial.print("   ");Serial.print(gyro_axis_cal[3]);Serial.print("   ");Serial.print(acc_axis_cal[1]);Serial.print("   ");Serial.println(acc_axis_cal[2]);
    // Serial.print("esc1:   ");Serial.print(esc_1);Serial.print("    esc_2:   ");Serial.print(esc_2);Serial.print("    esc3:   ");Serial.print(esc_3);Serial.print("    esc4:   ");Serial.println(esc_4);
    ...
    { //The motors are started.
        ...
        calculate_pid_angle();
        pid_roll_setpoint = pid_output_roll_angle;
        pid_pitch_setpoint = pid_output_pitch_angle;
        ...
        //    if (battery_voltage < 1680 && battery_voltage > 1000){                   //Is the battery connected?
...
void calculate_pid_angle()
{
    //Roll calculations
    pid_error_temp_angle = desired_angle_X - Total_angle[0];
    pid_i_mem_roll_angle += pid_i_gain_roll_angle * pid_error_temp_angle;
    if (pid_i_mem_roll_angle > pid_max_roll_angle)
        pid_i_mem_roll_angle = pid_max_roll_angle;
    else if (pid_i_mem_roll_angle < pid_max_roll_angle * -1)
        pid_i_mem_roll_angle = pid_max_roll_angle * -1;

    pid_output_roll_angle = pid_p_gain_roll_angle * pid_error_temp_angle + pid_i_mem_roll_angle + pid_d_gain_roll_angle * (pid_error_temp_angle - pid_last_roll_angle_d_error);
    if (pid_output_roll_angle > pid_max_roll_angle)
        pid_output_roll_angle = pid_max_roll_angle;
    else if (pid_output_roll_angle < pid_max_roll_angle * -1)
        pid_output_roll_angle = pid_max_roll_angle * -1;

    pid_last_roll_angle_d_error = pid_error_temp_angle;

    //Pitch calculations
    pid_error_temp_angle = desired_angle_Y - Total_angle[1];
    pid_i_mem_pitch_angle += pid_i_gain_pitch_angle * pid_error_temp_angle;
    if (pid_i_mem_pitch_angle > pid_max_pitch_angle)
        pid_i_mem_pitch_angle = pid_max_pitch_angle;
    else if (pid_i_mem_pitch_angle < pid_max_pitch_angle * -1)
        pid_i_mem_pitch_angle = pid_max_pitch_angle * -1;

    pid_output_pitch_angle = pid_p_gain_pitch_angle * pid_error_temp_angle + pid_i_mem_pitch_angle + pid_d_gain_pitch_angle * (pid_error_temp_angle - pid_last_pitch_angle_d_error);
    if (pid_output_pitch_angle > pid_max_pitch_angle)
        pid_output_pitch_angle = pid_max_pitch_angle;
    else if (pid_output_pitch_angle < pid_max_pitch_angle * -1)
        pid_output_pitch_angle = pid_max_pitch_angle * -1;

    pid_last_pitch_angle_d_error = pid_error_temp_angle;
}
